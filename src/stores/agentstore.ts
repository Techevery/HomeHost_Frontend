          ...
          set({
            ...
            agentInfo: {
              ...
              profile_picture:
                ...
                data.agent?.profile_picture,
              bank_name:
                data.bank_name || data.bankName || data.agent?.bank_name,
              account_number:
                data.account_number ||
                data.accountNumber ||
        ...
        try {
          ...
          if (!agentInfo?.isVerified && agentInfo?.status !== "VERIFIED") {
            return {
              success: false,
              message:
                "Your account needs to be verified before you can enlist properties. Please complete verification.",
            };
          }
          ...
          const hasAgentPercentage =
            agentPercentage !== undefined && agentPercentage !== null;

          if (hasMarkupPrice && hasAgentPercentage) {
            return {
              success: false,
              message:
                "Please select only one pricing option: either markup price OR agent percentage.",
            };
          }
          ...
          set({
            enlistedProperties:
              page === 1
                ...
                : [...get().enlistedProperties, ...(data.properties || data)],
            currentPropertyPage: page,
            totalProperties: data.totalCount || data.length || 0,
            hasMoreProperties:
              data.hasMore ||
              ((data.properties || data) &&
                (data.properties || data).length === limit),
          });
        } catch (error: any) {
          const errorMessage = handleApiError(
            error,
            "Failed to fetch enlisted properties.",
          );
          set({
            error: errorMessage,
          });
        ...
        try {
          ...
          set({
            agents: get().agents.map((agent) =>
              agent.id === agentId ? { ...agent, [field]: status } : agent,
            ),
          });
        } catch (error: any) {
          const errorMessage = handleApiError(
            error,
            "Failed to update agent verification.",
          );
          set({
            error: errorMessage,
          });
        ...
        try {
          ...
          const validatedProperties: PublicProperty[] = properties.map(
            (prop) => ({
              ...
              images: Array.isArray(prop.images)
                ...
                : ["/images/house1.svg"],
              status:
                prop.status === "unavailable" ? "unavailable" : "available",
          ...
          set({
            publicProperties:
              page === 1
                ? validatedProperties
                : [...get().publicProperties, ...validatedProperties],
            ...
            hasMorePublicProperties:
              result.hasMore !== undefined
                ? result.hasMore
                : validatedProperties.length === limit,
            loading: false,
          });
        } catch (error: any) {
